...
public class InsertsGenerator implements SqlGenerator {
    ...
    public String generate(String tableName, Map<Integer, List<String>> rowsMap) {
        ...
        for (Map.Entry<Integer, List<String>> e : rowsMap.entrySet()) {
            ...
            String value = OPENING_BRACKET + valuesList.stream()
                    .filter(val -> !columnsMap.get(valuesList.indexOf(val)).contains(ASTERISK))
                    .map(val -> getValue(columnsMap.get(valuesList.indexOf(val)), val))
                    .collect(Collectors.joining(COMMA)) + CLOSING_BRACKET;
            values.append(COMMA).append(value);
        }
        return INSERT_STATEMENT.replace(TABLE_NAME, tableName).replace(COLUMN_NAMES, columnsMap.values().stream().filter(c -> !c.contains(ASTERISK)).collect(Collectors.toList())
                .stream().map(this::getColumnName)
                .filter(Objects::nonNull)
                .collect(Collectors.joining(COMMA)))
                .replace(VALUES, values.toString().replaceFirst(COMMA, EMPTY));
    }
